...
- [IN PROGRESS] E2E UI click-through: scripts/e2e-smoke.mjs was made more tolerant of slow startups and now emits richer diagnostics. The ship gate script (scripts/ship.ps1) was updated to retry the E2E smoke up to 2 times and collect diagnostics on failure. A new ship workflow run was dispatched to GitHub Actions (run triggered). Local E2E still timed out here: electron first window did not appear — further investigation required on a real desktop or self-hosted runner.
