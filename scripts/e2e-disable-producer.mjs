...
const dataHome = join(tmpdir(), 'npz-e2e-disable-prod-' + Date.now());
...
try {
  ...
  // set the diag flag before any page code runs further
  await win.evaluate(() => { window.__npz_diag_disable_producer = true; console.log('[DIAG-FLAG] set __npz_diag_disable_producer = true'); });
  ...
  if ((await ta.count()) > 0) {
    await ta.fill('E2E disable producer probe');
  ...
  console.log('FINAL HEADINGS WITH PRODUCER_DISABLED:', JSON.stringify(headings));
